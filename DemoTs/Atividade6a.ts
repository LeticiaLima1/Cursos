...
function apresentarPessoa(idBio: any){ //função que mostrará a pessoa

    const idFigura: Number = Number(idBio.value); // Cria uma variável do tipo número, recebe o valor do id transformando-o em número
    ...
    listaFiguras.forEach(figura =>{ //laço para percorrer a lista
        ...
        if(figura.id === idFigura){ //compara o id de quem foi escolhido com o id da Figura da classe
            document.getElementById("Bio").innerHTML = figura.apresentar(); //imprime a bio da Figura
        }
